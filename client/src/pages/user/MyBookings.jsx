...
import { Lock, Car, Calendar, MapPin, X, Eye, Shield, Zap, MessageSquare, Fingerprint } from 'lucide-react';
    ...
    return (
        <div className='p-8 md:p-12 h-screen overflow-y-auto custom-scrollbar bg-[#050505] pb-32'>
            <div className='max-w-6xl mx-auto'>
                {/* HEADER AREA */}
                <PortalTitle title='My Bookings' subTitle={`Welcome back, ${userData?.name}. Oversee your active fleet and premium rentals.`} />
                                        ...
                                        <div className='flex-1 flex flex-col justify-between py-2'>
                                            ...
                                            <div className='mt-12 pt-10 border-t border-white/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8'>
                                                ...
                                                <div className='flex items-center gap-6 w-full sm:w-auto'>
                                                    <Link to={`/car-details/${booking.car?._id || booking.car}`} className='flex-1 sm:flex-none'>
                                                        <button className='w-full sm:w-auto h-16 px-10 bg-white/5 hover:bg-white/10 text-[9px] font-black uppercase tracking-[0.4em] rounded-full transition-all border border-white/5 flex items-center justify-center gap-3'>
                                                            <Eye className='w-4 h-4' /> View Vehicle
                                                        </button>
                                                    </Link>
                                                    {booking.status === 'Confirmed' && (
                                                        <Link to={`/chat/${booking._id}`} className='flex-1 sm:flex-none'>
                                                            <button className='w-full sm:w-auto h-16 px-10 bg-primary/10 hover:bg-primary text-primary hover:text-white text-[9px] font-black uppercase tracking-[0.4em] rounded-full transition-all border border-primary/20 flex items-center justify-center gap-3'>
                                                                <MessageSquare className='w-4 h-4' /> Chat with Owner
                                                            </button>
                                                        </Link>
                                                    )}
                                                    <Link to="/user/verify" className='flex-1 sm:flex-none'>
                                                        <button className='w-full sm:w-auto h-16 px-10 bg-white/5 hover:bg-white/10 text-white/40 hover:text-white text-[9px] font-black uppercase tracking-[0.4em] rounded-full transition-all border border-white/5 flex items-center justify-center gap-3'>
                                                            <Fingerprint className='w-4 h-4' /> Edit Identity
                                                        </button>
                                                    </Link>
                                                    {booking.status === 'Confirmed' && (
                                                        <button 
